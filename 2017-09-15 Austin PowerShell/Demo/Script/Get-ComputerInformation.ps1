function Get-ComputerInformation {
    ...
    # Get IP Information of the system
    $WQLQuery = 'select ipaddress,dnsserversearchorder,defaultipgateway from win32_networkadapterconfiguration where IPEnabled = "True"'
    $IPInfo = Get-CimInstance -Query $WQLQuery
    
    $HashTable.Ipaddress = $IPInfo.IPAddress -notmatch ':' -join ', '
    $HashTable.DnsServer = $IPInfo.dnsserversearchorder -join ', '
    $HashTable.Gateway = $IPInfo.defaultipgateway -join ', '
