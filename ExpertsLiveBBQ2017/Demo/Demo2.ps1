...
$ConfSplat = @{
    ...
    RoleDefinitions     = @{'Awesome\HelpDeskUser' = @{RoleCapabilities = 'HelpDeskUser'}}
...
#region GUI AD Management
# Start-VM
Invoke-Command -Session $HelpDeskSession -ScriptBlock {Get-ADUser -Filter *} |
Out-GridView -Title 'Select User to Unlock' -PassThru |
ForEach-Object {
    $CurrentUser = $_
    Invoke-Command -Session $HelpDeskSession {
        Unlock-ADAccount -WhatIf $using:CurrentUser.Name
    }
}
#endregion

# Show logging
ls C:\Transcription\20170614 | sort lastwritetime | select -l 1 | sls '\(Unlock-ADAccount\)\: "'
